...
if (preg_match('/\/v1\/mock\/wallets\/(?<wallet_id>\d+)\/apitoken$/i', $path, $m)) {
    ...
    echo response($resp);
    return;
} else if (preg_match('/\/v1\/mock\/currency\/(?<currency>\d+)\/contract\/get-multiple-tokenuri$/i', $path, $m)) {
    $uri = '/v1/sofa/currency/'.$m['currency'].'/contract/get-multiple-tokenuri';
    $resp = make_request($m['wallet_id'], $method, $uri, $query, $post_data);
    log_access($uri, $resp);
    echo response($resp);
    return;
} else if (preg_match('/\/v1\/mock\/wallets\/(?<wallet_id>\d+)\/signtransaction$/i', $path, $m)) {
    $uri = '/v1/sofa/wallets/'.$m['wallet_id'].'/signtransaction';
    $resp = make_request($m['wallet_id'], $method, $uri, $query, $post_data);
    log_access($uri, $resp);
    echo response($resp);
    return;
} else if (preg_match('/\/v1\/mock\/healthcheck$/i', $path, $m)) {
    $uri = '/v1/sofa/healthcheck';
    $resp = make_request(null, $method, $uri, $query, $post_data);
    log_access($uri, $resp);
    echo response($resp);
    return;
}
